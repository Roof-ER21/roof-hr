...
Should I proceed with scheduling this interview?`,
  ...
  private async handleEmployeeUpdate(message: string, user: User): Promise<ActionResult | null> {
    try {
      // Extract employee name and update details
      const employeeMatch = message.match(/(?:update|change|modify)\s+([A-Za-z]+(?:\s+[A-Za-z]+)?)/i);
      const updateMatch = message.match(/(?:set|change|update)?\s*(?:their|the)?\s*(email|phone|department|position|role|status)\s+(?:to|as)?\s+([^\s]+.*?)(?:\.|$)/i);
      
      if (!employeeMatch) {
        return {
          success: false,
          message: "Please specify which employee to update."
        };
      }
      ...
      const employee = employees.find(e => {
        ...
        return fullName.includes(employeeName) || 
               e.firstName.toLowerCase() === employeeName ||
               e.lastName.toLowerCase() === employeeName;
      });
      ...
      const updates: any = {};
      if (updateMatch) {
        const field = updateMatch[1].toLowerCase();
        ...
        switch(field) {
          ...
          case 'position':
            ...
            break;
          case 'role':
            updates.role = value.toUpperCase();
            break;
          case 'status':
      ...
      if (Object.keys(updates).length === 0) {
        return {
          success: false,
          message: "Please specify what to update (email, phone, department, position, role, or status)."
        };
      }
      
      await storage.updateUser(employee.id, updates);
      
      return {
        success: true,
        message: `✅ Successfully updated ${employee.firstName} ${employee.lastName}.\n\nUpdated fields: ${Object.keys(updates).join(', ')}`,
        data: { employeeId: employee.id, updates }
      };
    } catch (error) {
      return {
        success: false,
